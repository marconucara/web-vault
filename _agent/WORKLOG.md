...
| 2026-08-01 | (this commit) | main | ADR 0035 closure (plan todo/0001) | Closed the starter-template item. The `web-vault` side shipped earlier in `c3098da` (deleted `templates/base/`, README repointed); this closes it now that `marconucara/web-vault-template` (`727e7de`) is restored as the one-click entry point and a live button test from its root reaches the configuration screen. ADR 0035 → Implemented (r5), INDEX updated, plan → `done/2026-08-01-template-separate-repo.md`. Also swapped the two post-deploy steps in the template's `README.md` and `welcome.md` so Access (make it private) comes before `GITHUB_TOKEN` — the site is public until Access is on. |
