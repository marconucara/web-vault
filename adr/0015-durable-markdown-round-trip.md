...
6. A block nested under a list item — paragraph, blockquote, table, heading or
   ...
   round trip.
7. A note parses to the block structure its markdown describes, and a pre-process
   pass never rewrites code: link syntax written inside an inline code span or a
   fenced block stays literal, and no pass matches across a line boundary. A
   byte-identical round trip does not satisfy this on its own — a pass and its
   inverse can cancel out while the editor still shows the wrong blocks.
...
| 2026-08-06 | r7 | marco | Added criterion 7 after a defect that criterion 1 could not have caught: the wikilink and media-link pre-processors matched across line boundaries and did not skip code, so an unmatched `[[` inside a code span — the shape a note documenting the syntax naturally has — started a match that ran to the next `]]` anywhere later in the note and collapsed every heading and paragraph in between into one inline token. The bytes still round-tripped, because each post pass reverses its own pre pass symmetrically, which is why an earlier audit found no defect and downgraded the item to test coverage; the loss was in the block structure the editor displays, and it became permanent as soon as the user edited the mangled block. Both matchers are now bounded to a single line, and both passes apply only outside fenced blocks and inline code spans. The pipeline order is unchanged: `preProcessFences` rewrites only the opening fence line, so running it first would not have protected fence content, and nothing but the pass itself can protect a code span. Criterion 7 states the general rule — structure is part of the contract, not only the bytes — so the next such pass is written against it. |
