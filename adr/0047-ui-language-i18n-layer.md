...
combination, not an edge case. So the resolved locale drives `Intl` by default,
and the formatting locale can be set independently through its own seam.
...
language level — region subtags are ignored. The resolved locale also drives
locale-dependent formatting of dates, numbers and relative times, and a separate
formatting locale may be supplied independently of the interface language. Both
...
10. The resolved locale is exposed as the locale used for `Intl`-based
    formatting of dates, numbers and relative times.
11. A formatting locale can be supplied at boot and switched at runtime,
    independently of the interface language; when it is absent, formatting
...
- **Region-aware locale codes** (`en-GB`, `pt-BR`). Rejected: with two languages
  it adds a matching hierarchy that carries no content, since no two regions
  would differ in a single string.
